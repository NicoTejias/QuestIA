...
const CustomPassword = Password({
  profile(params) {
    ...
    if (role === "teacher") {
        ...
        if (!isAllowed) {
            // Permitimos el paso para el modo Demo, se manejará en storeUser
        }
    }
...
const CustomGoogle = Google({
  ...
  profile(profile) {
    ...
    if (!isAllowed) {
      // Allow for Demo mode. The exact role assignment happens in storeUser/profile but for Clerk/Auth it needs to match.
      // Wait, we don't have storeUser, auth.ts runs profile().
    }

    const isTeacherEmail = email.includes("@profesor.duoc.cl") || email.includes("@duoc.cl") || email.includes("@questia.cl");
    
    let role = isTeacherEmail ? "teacher" : "student";
    if (!isAllowed) {
      role = "demo_teacher";
    }
