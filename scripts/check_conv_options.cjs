    ...
    ws.on('open', () => {
      const code = `
        (async () => {
          ...
          // Check options endpoint for conversations
          let options = null;
          try {
            const r = await $http.get('/learn/api/v1/courses/_734484_1/conversations/options');
            options = r.data;
          } catch(e) {
            options = { error: e.status, msg: e.data };
          }
          return { options };
        })()
      `;
