    ...
    ws.on('open', () => {
      const code = `
        (async () => {
          ...
          const res = await $http.get('/learn/api/v1/users/me/memberships?expand=course.effectiveAvailability&limit=50');
          const courses = [];
          for (const m of res.data.results || []) {
            courses.push({
              id: m.courseId,
              name: m.course?.name || m.course?.displayName,
              code: m.course?.courseNumber,
              ultra: m.course?.ultraStatus
            });
          }
          return courses.filter(c => c.name).slice(0, 15);
        })()
      `;
      ws.send(JSON.stringify({ id: 1, method: 'Runtime.evaluate', params: { expression: code, awaitPromise: true, returnByValue: true } }));
    });
    ws.on('message', m => {
      console.log('Courses:', JSON.stringify(JSON.parse(m).result?.result?.value, null, 2));
      ws.close();
    });
  });
});
