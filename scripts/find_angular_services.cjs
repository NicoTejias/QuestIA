    ...
    ws.on('open', () => {
      const code = `
        (async () => {
          ...
          // Check if there is a course message service or conversation factory
          let services = [];
          if (injector) {
            try {
              services = ['$http', 'courseMessagesService', 'conversationsService', 'conversationService', 'courseConversationsService'].filter(s => {
                try { return !!injector.get(s); } catch(e) { return false; }
              });
            } catch(e) {}
          }
          return { services };
