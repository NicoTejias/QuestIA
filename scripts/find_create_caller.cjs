    ...
    ws.on('open', () => {
      const code = `
        (async () => {
          ...
          // Find where createConversation is called
          const target = '.createConversation(';
          const matches = [];
          let pos = 0;
          while ((pos = txt.indexOf(target, pos)) !== -1) {
            matches.push(txt.substring(Math.max(0, pos - 150), Math.min(txt.length, pos + 400)));
            pos += target.length;
