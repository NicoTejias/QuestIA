    ...
    ws.on('open', () => {
      const code = `
        (async () => {
          ...
          const pos = 3267114;
          const posEnd = txt.indexOf('});', pos);
          const exportStr = txt.substring(pos, posEnd);
          
          // Let's find rM in exportStr
          const idx = exportStr.indexOf(':()=>rM');
          return {
            hasRM: idx !== -1,
            snippet: idx !== -1 ? exportStr.substring(idx - 10, idx + 10) : 'not found'
          };
        })()
      `;
