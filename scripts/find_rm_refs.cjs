    ...
    ws.on('open', () => {
      const code = `
        (async () => {
          ...
          const pos = 3291207;
          // Look down from pos for where rM is exported or called
          const exportRegex = /rM\b/g;
          exportRegex.lastIndex = pos + 10;
          const matches = [];
          let m;
          while ((m = exportRegex.exec(txt)) !== null) {
            matches.push({
              pos: m.index,
              snippet: txt.substring(Math.max(0, m.index - 50), Math.min(txt.length, m.index + 200))
            });
