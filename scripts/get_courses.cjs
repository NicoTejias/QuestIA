...
http.get('http://192.168.0.202:9222/json', (res) => {
  ...
  res.on('end', () => {
      ...
      const code = `
        (() => {
          const links = Array.from(document.querySelectorAll('a'))
            .filter(a => a.href && (a.href.includes('/ultra/courses/') || a.href.includes('/ultra/course/')))
    ...
    ws.on('message', m => {
      console.log('Courses found:', JSON.stringify(JSON.parse(m).result?.result?.value, null, 2));
      ws.close();
    });
  });
});
