    ...
    ws.on('open', () => {
      const code = `
        (async () => {
          ...
          const target = 'let eJ=ed.q7;eo.A.factory(eJ';
          const pos = txt.indexOf(target);
          // Look for ed definition before pos
          const edDef = txt.lastIndexOf('ed=', pos);
          return txt.substring(edDef, edDef + 600);
        })()
      `;
