...
http.get('http://192.168.0.202:9222/json', (res) => {
  ...
  res.on('end', () => {
    ...
    ws.on('open', () => {
      const code = `
        new Promise((resolve) => {
          ...
          xhr.open('GET', '/learn/api/v1/courses/conversations/counts');
          ...
          xhr.onerror = (e) => resolve({ error: 'XHR error' });
          xhr.send();
        })
      `;
    ...
    ws.on('message', m => {
      console.log('XHR Result:', JSON.stringify(JSON.parse(m).result?.result?.value, null, 2));
      ws.close();
    });
  });
});
